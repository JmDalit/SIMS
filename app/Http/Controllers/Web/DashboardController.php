...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        return Inertia::render('Web/dashboardPage', [
            ...
            'campuses_details' => Auth::user()->role_array['name'] != 'Administrator' && SchoolCampuses::select('id', 'generated_name', 'school_id', 'name')->where(['agency_id' => Auth::user()->profile->agency_id, 'is_delete' => false]) ? SchoolCampuses::select('id', 'generated_name', 'school_id', 'name')->where(['agency_id' => Auth::user()->profile->agency_id, 'is_delete' => false])
                ->with([
                    'school' => fn($q) => $q
                        ->select('id', 'shortcut')
