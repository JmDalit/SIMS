    ...
    <Drawer v-model:visible="modelValue" position="full" :pt="{
        ...
        <template #default>
            <div class="flex flex-col lg:flex-row w-full pt-5 gap-3 h-full">
                ...
                <div class="lg:flex-7">
                    <Panel class="!rounded-xl" v-if="selectedTab.key == 1"
                        :pt="{ header: '!border-b-1 !border-gray-300 !border-dashed !p-0', }">
                        <template #header>
                            <div class="p-2 w-full flex justify-between">
                                <div class="flex items-center gap-2">
                                    ...
                                    <h3 class="text-lg font-bold text-nowrap">Personal Records</h3>
                                </div>
                                <div class="flex w-full justify-end">
                                    ...
                                    <div class="flex items-center gap-2" v-else>
                                        <DefaultButton :icon="TablerIcons.IconUserCancel
                                            " label="Cancel Edit" size="small" severity="danger"
                                            @click="editBtn.info = false" outlined class-name="!rounded-xl !px-5" />
                                        <DefaultButton :icon="TablerIcons.IconUserCheck
                                            " raised label="Save this details" size="small"
                                            class-name="!rounded-xl !px-5" />
                                    </div>
                                </div>
                            </div>
                        ...
                        <template #default>
                            <div class="w-full flex flex-col pt-5 gap-3">
                                <div class="flex items-center gap-3">
                                    <TextInput v-model="personalInfo.first_name
                                        " label="First Name" :disabled="!editBtn.info"></TextInput>

                                    <TextInput v-model="personalInfo.middle_name
                                        " label="Middle Name" :disabled="!editBtn.info"></TextInput>

                                </div>
                                <div class="flex items-center gap-3">
                                    <TextInput v-model="personalInfo.last_name
                                        " label="Last Name" :disabled="!editBtn.info"></TextInput>
                                ...
                                <div class="flex items-center gap-2">
                                    <DatePickerInput v-model="personalInfo.birth_date
                                        " label="Birth Date" :disabled="!editBtn.info"></DatePickerInput>
                                    <TextInput v-model="personalInfo.birth_place
                                        " label="Birth Place" :disabled="!editBtn.info"></TextInput>
                                </div>
                                <div class="flex items-center gap-2">
                                    <TextInput v-model="personalInfo.religion" label="Religion" capitalize
                                        :disabled="!editBtn.info"></TextInput>
                                    <TextInput v-model="personalInfo.civil_status
                                        " capitalize label="Civil Status" :disabled="!editBtn.info">
                                    </TextInput>
                                </div>
...
const selectedTab = ref({ label: "Personal Records", icon: "IconUserQuestion", key: 1 },);
const editBtn = ref({
    info: false,
    tr: false,
    stipend: false,
...
const tabs = ref([
    ...
    { label: "Activit logs", icon: "IconWood", key: 4 },
]);
